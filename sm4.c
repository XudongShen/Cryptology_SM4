...
void sm4_setKey_encrypt(sm4_context *context, unsigned char key[16]) {
	context->mode = SM4_ENCRYPT;
	sm4_setKey(context->rk, key);
}

void sm4_setKey_decrypt(sm4_context *context, unsigned char key[16]) {
	int i = 0;
	unsigned long tmp;
	context->mode = SM4_DECRYPT;
	sm4_setKey(context->rk, key);
	for (; i < 16; i++) {
		tmp = context->rk[i];
